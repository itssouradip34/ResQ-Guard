...
def simulate_junction_scenario(
    ...
    # Real volume if we have a forecast; otherwise a conservative default
    # rather than a fabricated "busy" number.
    ...
    def webster_delay(cycle_sec: float, green_sec: float, vol: float, cap: float) -> float:
        """Returns average per-vehicle delay in seconds."""
        if cap <= 0:
            return float("inf")
        green_ratio = min(0.95, max(0.05, green_sec / cycle_sec))
        x = min(0.98, vol / (cap * green_ratio))  # volume/capacity (degree of saturation)
    ...
    baseline_green = DEFAULT_CYCLE_LENGTH_SEC * 0.45  # assume ~45% green split baseline
